 ...
 * POJO转换类，目前用于DTO、BO、PO之间相同属性字段之间的复制
 */
public class CopyUtils {
     ...
     * @return
     */
    public static <T> T copyBeanWithNull(Object source, Class<T> targetClazz) {
        if (null == source) {
            return null;
        }
     ...
     * @return
     */
    public static <T, E> List<T> copyListWithNull(List<E> sourceList, Class<T> targetClazz) {
        if (null == sourceList) {
            return null;
        } else if (0 == sourceList.size()) {
            return new ArrayList<>();
